...
export const handlePresence = (socket, io) => {
  socket.on(SOCKET_EVENTS.PRESENCE_JOIN, ({ workspaceId }) => {
  ...
  socket.on(SOCKET_EVENTS.PRESENCE_LEAVE, ({ workspaceId }) => {
    if (!workspaceId) return;
 ...
 */
export const handleCollaboration = (socket, io) => {
  socket.on(SOCKET_EVENTS.TYPING_START, ({ taskId }) => {
    if (!taskId) return;
  ...
  socket.on(SOCKET_EVENTS.TYPING_STOP, ({ taskId }) => {
    if (!taskId) return;
 ...
 */
export const handleRoomContext = (socket) => {
  ...
  socket.on('room:join:project', ({ projectId }) => {
    if (projectId) socket.join(SOCKET_ROOMS.PROJECT(projectId));
  });

  socket.on('room:join:task', ({ taskId }) => {
