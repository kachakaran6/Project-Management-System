...
const presenceMap = new Map(); // key: workspaceId, value: Set(userIds)
 ...
 */
export const leaveWorkspacePresence = (workspaceId, userId) => {
  if (presenceMap.has(workspaceId)) {
    presenceMap.get(workspaceId).delete(userId);
    if (presenceMap.get(workspaceId).size === 0) {
 ...
 * @param {string} workspaceId 
 */
export const getActiveUsers = (workspaceId) => {
  return presenceMap.has(workspaceId) ? Array.from(presenceMap.get(workspaceId)) : [];
};
