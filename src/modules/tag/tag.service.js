 ...
 */
export const getTags = async (organizationId, workspaceId = null) => {
 ...
 */
export const assignTagToTask = async (taskId, tagId, organizationId) => {
  try {
    const taskTag = await TaskTag.create({ taskId, tagId, organizationId });
    return taskTag;
  } catch (error) {
    if (error.code === 11000) return { success: true, message: 'Already tagged' };
    throw error;
  }
};
 ...
 */
export const removeTagFromTask = async (taskId, tagId, organizationId) => {
 ...
 */
export const deleteTag = async (tagId, organizationId) => {
