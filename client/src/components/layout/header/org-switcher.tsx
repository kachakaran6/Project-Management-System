...
import { Building2, Check, ChevronDown, User2, Loader2, Info } from "lucide-react";
...
function SingleOrgChip({ org }: { org: OrganizationMembership }) {
  return (
    <TooltipProvider delayDuration={300}>
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            className={cn(
              ...
              "text-sm backdrop-blur-sm cursor-default select-none",
            )}
          >
        ...
        <TooltipContent side="bottom" className="flex items-center gap-1.5 text-xs">
          ...
          You are part of 1 organization
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
...
export function SmartOrgSwitcher() {
  ...
  switch (mode) {
    ...
    case "single":
      return <SingleOrgChip org={activeOrg!} />;
