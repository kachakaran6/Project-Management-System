      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button 
            type="button"
            className={cn(
              "inline-flex items-center gap-1.5 h-6 px-2.5 rounded-button border border-dashed transition-all text-[11px] font-bold shrink-0 outline-none focus:outline-none focus:ring-0 focus-visible:ring-0",
        ...
        <PopoverContent 
          className="w-64 p-0 rounded-button border border-border/40 bg-popover/95 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] overflow-hidden animate-in fade-in zoom-in-95 duration-200" 
          align="start" 
          side="bottom" 
          sideOffset={12}
        >
                ...
                {tags.map((tag) => {
                  ...
                  return (
                    <CommandItem
                      ...
                      className={cn(
                        "flex items-center justify-between py-2.5 px-3 rounded-button cursor-pointer transition-all m-0.5 border border-transparent",
                        isSelected ? "bg-primary/20 border-primary/10" : "hover:bg-white/5"
                      )}
                    >
                      <div className="flex items-center gap-3.5 min-w-0">
                        <div 
                          className="flex h-8 w-8 items-center justify-center rounded-button shrink-0 shadow-[0_4px_12px_rgba(0,0,0,0.2)] transition-all group-hover:scale-105"
